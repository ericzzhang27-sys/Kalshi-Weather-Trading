...
3. **Fail closed in trading code.** Missing data, stale timestamps, ambiguous bucket mappings, or failed validations must produce `NO_TRADE` / an error — never a guessed value or order.
...
Model search / ablation utilities exist under `scripts/` (`search_ngboost_model_space.py`, `run_focused_nll_brier_search.py`, `run_refined_skew_search.py`, `run_ngboost_feature_ablation.py`, `compare_ngboost_distributions.py`) — use only for research branches, not to silently replace the production artifact.
